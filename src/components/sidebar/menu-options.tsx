...
const MenuOptions = ({details, id, sidebarLogo, sidebarOpt, subAccounts, user, defaultOpen}: Props) => {
  ...
  return (
    <Sheet modal={false} 
        ...
        <SheetContent aria-labelledby='Menu Options' showX={!defaultOpen} side={'left'} className={clsx('bg-background/80 backdrop-blur-xl fixed top-0 border-r-[1px] p-6', {'hidden md:inline-block z-0 w-[300px]': defaultOpen, 'inline-block md:hidden z-[100] w-full': !defaultOpen})}>
            <SheetTitle className='hidden'>Menu Options</SheetTitle>
            <div>
                                ...
                                {(user?.role === "AGENCY_OWNER" || user?.role === "AGENCY_ADMIN") && user?.Agency && <CommandGroup heading="Agency">
                                    <CommandItem className='!bg-transparent my-2 text-primary border-[1px] border-border p-2 rounded-md hover:!bg-muted cursor-pointer transition-all'>
                                        {defaultOpen ? <Link href={`/agency/${user?.Agency?.id}`} className='flex gap-4 w-full h-full'>
                                            ...
                                            <div className='flex flex-col flex-1'>{user?.Agency?.name}
                                ...
                                <CommandGroup heading="Accounts">
                                    {!!subAccounts ? subAccounts.map((subaccount) => 
                                        ...
                                        {defaultOpen ? <Link href={`/subaccount/${subaccount.id}`} className='flex gap-4 w-full h-full'>
                                            ...
                                            <div className='flex flex-col flex-1'>{subaccount.name}
